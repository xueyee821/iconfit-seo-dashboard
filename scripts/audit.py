"""
iconfit.asia Weekly SEO Audit Script
Fetches real data from GSC, GA4, and technical checks
"""
import os, json, datetime, urllib.request, ssl, subprocess, re
...
GA4_ID     = os.environ.get('GA4_PROPERTY_ID', 'properties/508017836')
GSC_URL    = os.environ.get('GSC_SITE_URL', 'sc-domain:iconfit.asia')
SITE       = 'https://iconfit.asia'
...
def fetch_gsc(service, days=90):
    ...
    return {
        ...
        'countries': [
            {'country': r['keys'][0].upper(), 'clicks': r['clicks']}
            for r in countries
        ],
        'devices': [
...
def fetch_technical():
    ...
    results['security_headers']  = sum([
        results['has_hsts'], results['has_xframe'],
    ...
    results['has_llms_txt'] = len(llms) > 50 and 'Iconfit' in llms

    # AI crawler access
    gptbot_headers   = curl(SITE, ['-sI', '-A', 'GPTBot'])
    claudebot_headers = curl(SITE, ['-sI', '-A', 'ClaudeBot'])
    results['gptbot_allowed']    = 'HTTP/2 200' in gptbot_headers or 'HTTP/1.1 200' in gptbot_headers
    results['claudebot_allowed'] = 'HTTP/2 200' in claudebot_headers or 'HTTP/1.1 200' in claudebot_headers
...
# ── Score Calculator ──────────────────────────────────────────
def calculate_scores(gsc, ga4, tech):
    # Technical score
    tech_score = 54
...
def main():
    ...
    print(f"   ✓ llms.txt: {'OK' if tech['has_llms_txt'] else 'MISSING'}")
    print(f"   ✓ REST API: {'EXPOSED' if tech['rest_api_exposed'] else 'PROTECTED'}")

    scores = calculate_scores(gsc, ga4, tech)
    ...
    print("✅ Data saved to data/latest.json")
